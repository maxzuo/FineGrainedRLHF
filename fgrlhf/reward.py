...
class BasicReward(metaclass=abc.ABCMeta):
  ...
  def kl_penalize_reward(self, results):
    ...
    flattened_rewards = torch.tensor(
        [r + [0.] * (RL - len(r)) for r in raw_rewards], device=logprobs.device)
...
class PreferenceReward:
  ...
  def get_reward(
    ...
    for batch_idx, (meta, gen_text) in enumerate(zip(metadata,
                                                     generated_texts)):
...
class BaselineReward(BasicReward):
  ...
  def __init__(
    ...
    self.baseline_reward = PreferenceReward(
        tokenizer,
        baseline_model_ckpt,
  ...
  def get_reward(
    ...
    rewards = self.baseline_reward.get_reward(
        prompts_input_ids,
        prompts_attention_mask,
        generated_input_ids,
        generated_attention_mask,
...
class SelectiveFineGrainedReward(BasicReward):
  ...
  def get_finegrained_reward(
    ...
    if self.completeness_reward:
      fine_grained_rewards = self.completeness_reward.get_reward(
          prompts_input_ids,
          prompts_attention_mask,
          generated_input_ids,
          generated_attention_mask,
    ...
    elif self.verbosity_reward:
      fine_grained_rewards = self.verbosity_reward.get_reward(
          prompts_input_ids,
          prompts_attention_mask,
          generated_input_ids,
          generated_attention_mask,
