...
class BasicReward(metaclass=abc.ABCMeta):
    ...
    def eval_metrics(
        ...
        training_rewards = self.get_reward(prompts_input_ids,
        ...
        return {
            "eval/rewards":
            [np.sum(sublist) for sublist in training_rewards['rewards/raw']],
        }
    ...
    def kl_penalize_reward(self, results):
        ...
        flattened_rewards = torch.tensor(
            [r + [0.] * (RL - len(r)) for r in raw_rewards],
...
class PreferenceReward:
    ...
    def get_reward(
        ...
        for batch_idx, (meta,
                        gen_text) in enumerate(zip(metadata, generated_texts)):
        ...
        fine_grained_reward = [[0.] * (l - 1) + [
            ((r - self.mean) / self.std) * self.scale + self.bias
        ] for r, l in zip(sequence_level_reward,
                          torch.sum(generated_attention_mask, dim=1).tolist())]
...
class SubSentenceVerbosityReward:
    ...
    def __init__(
        ...
        self.nf_reward_model = LongformerForTokenClassification.from_pretrained(
            model_ckpt)
    ...
    def find_sep_position(self, input_ids):
        return torch.nonzero(input_ids == self.sep_id,
                             as_tuple=False).squeeze().tolist()

    def process_one_generation(self, long_text, policy_text_len):

        sentence_end_char_idxs = split_text_to_subsentences(
            long_text, self.nlp)

        sentences = [
            long_text[sentence_end_char_idxs[i]:sentence_end_char_idxs[i + 1]]
            for i in range(len(sentence_end_char_idxs) - 1)
        ]
        ...
        for sent_idx in range(len(sentences)):
            tokens = self.policy_tokenizer.tokenize(
                long_text[:sentence_end_char_idxs[sent_idx + 1]])
        ...
        # in case overlength
        sentence_end_indices = [
            min(item, policy_text_len - 1) for item in sentence_end_indices
        ]
    ...
    def get_reward(
        ...
        policy_inputs_lens = torch.sum(generated_attention_mask,
                                       dim=1).tolist()
        ...
        with torch.no_grad():
            ...
            inputs = self.reward_tokenizer(
                [s.split() for s in batch_nf_reward_inputs],
                truncation=True,
                padding=True,
        ...
        for text_idx, generated_text in enumerate(generated_texts):
            ...
            for i, end_idx in enumerate(policy_sentence_end_indices):
                nf_error_type = torch.argmax(
                    sentence_nf_reward_probs[i][[1, 2]]).item()
        ...
        return {
            ...
            "n_sub_sentences":
            [len(item) for item in batch_sentence_end_indices],
...
class FactualityReward:
    ...
    def __init__(
        ...
        self.f_reward_model = LongformerForTokenClassification.from_pretrained(
            model_ckpt)
    ...
    def get_reward(
        ...
        with torch.no_grad():
            ...
            # to align with the token classification model
            inputs = self.reward_tokenizer(
                [s.split() for s in batch_f_reward_inputs],
                truncation=True,
                padding=True,
                is_split_into_words=True,
                return_tensors="pt",
        ...
        for text_idx, generated_text in enumerate(generated_texts):
            ...
            for i, end_idx in enumerate(policy_sentence_end_indices):
                ...
                f_error_type = torch.argmax(
                    sentence_f_reward_probs[i][[0, 2]]).item()
...
class FineGrainedReward(BasicReward):
    ...
    def get_finegrained_reward(self, prompts_input_ids, prompts_attention_mask,
        ...
        factuality = self.factuality_reward.get_reward(
        ...
        for text_idx, generated_text in enumerate(generated_texts):
            ...
            fine_grained_reward = [
                a + b + c for a, b, c in
                zip(verbosity_rewards[text_idx], factuality_rewards[text_idx],
    ...
    def eval_metrics(
        ...
        for text_idx, generated_text in enumerate(generated_texts):
            ...
            n_verbosity_correct = finegrained_rewards_output[
                'n_verbosity_correct'][text_idx]
            relevance_ratios.append(n_verbosity_correct / n_sub_sentence)

            # factuality reward
            n_sentence = n_sentences[text_idx]
            n_factuality_correct = finegrained_rewards_output[
                'n_factuality_correct'][text_idx]
            factuality_ratios.append(n_factuality_correct / n_sentence)

            # completeness reward
            completeness_rewards.append(
                (finegrained_rewards_output['completeness_rewards'][text_idx]
                 [-1] - self.completeness_reward_bias) /
                self.completeness_reward_scale)
        ...
        output.update({
            ...
            "eval/rewards": [
                np.sum(sublist)
                for sublist in finegrained_rewards_output['rewards']
            ],
            "eval/relevance_ratios":
            relevance_ratios,
            "eval/factuality_ratios":
            factuality_ratios,
            "eval/completeness_rewards":
            completeness_rewards,
            "eval/n_sub_sentences":
            n_sub_sentences,
            "eval/n_sentences":
            n_sentences,
            "eval/lengths":
...
class BaselineReward(BasicReward):
    ...
    def get_reward(
        ...
        rewards = self.baseline_reward.get_reward(prompts_input_ids,
                                                  prompts_attention_mask,
                                                  generated_input_ids,
                                                  generated_attention_mask,
                                                  generated_texts, metadata)
    ...
    def eval_metrics(
        ...
        output = {}
        baseline_rewards_output = self.get_reward(
            prompts_input_ids, prompts_attention_mask, generated_input_ids,
            generated_attention_mask, generated_texts, metadata)['rewards/raw']
        # compute rouge scores
        rouge_scores = get_rouge_scores(generated_texts,
                                        [m['references'] for m in metadata])
        ...
        output.update({
            ...
            rouge_scores,
            "eval/rewards":
            [np.sum(sublist) for sublist in baseline_rewards_output],
            "eval/lengths":
            generation_lens
        })
